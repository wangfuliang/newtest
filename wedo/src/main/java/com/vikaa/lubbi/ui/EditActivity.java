...
public class EditActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initDateTimePicker() {
        ...
        calendar.setTime(new Date(DateUtils.dateToLong("yyyy-MM-dd HH:mm:ss", entity.getTime())));
    ...
    private void create() {
        ...
        String _time = time.getText().toString().trim();
        if (_time.length() != 19)
            _time = time.getText().toString().trim() + ":00";
        ...
        if (check(_title, _mark)) {
            ...
            httpUtils.send(HttpRequest.HttpMethod.POST, MyApi.updateRemind, params, new RequestCallBack<String>() {
                @Override
                public void onStart() {
